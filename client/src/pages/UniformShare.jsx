...
export default function UniformShare() {
  const { id } = useParams();
  ...
  async function startChallenge() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/uniforms/game/${id}/rematch`, { method: 'POST' });
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen gap-6 px-4 py-10">
      <h1 className="text-3xl font-bold text-yellow-400">👕 Loading uniform challenge…</h1>
      <p className="text-gray-400 text-center max-w-md">
        You are getting the same uniforms set.
      </p>
      {error && (
        <>
